...
Everything the gate needs is on one cached row, published by the plans-cacher: the account's max
quota (from upstream) and its current usage (computed by us, in the background). So the whole gate
is one Redis HGET and a comparison -- no database work on the request path at all.
...
WHAT THAT COSTS, stated plainly because it is a deliberate choice and not an oversight:
  ...
  * A denial is NOT re-checked against a live count. A customer who deletes data to get back under
    ...
    of relying on the cache alone; the refresh interval is the only lever on it.
...
FAILURE POSTURE -- the two cases are deliberately different:
  ...
  * The lookup succeeds and says "this account is on plan X", but X's quota is unknown (cold cache,
    unpriced plan): ALLOW, loudly. A positively identified paying customer must never be blocked
    because our cache has not warmed up.
"""
...
def evaluate_quota(quota: PlanQuota, incoming_bytes: int, *, enforcing: bool = True) -> PlanDecision:
    """Decide whether this write fits inside the account's quota.
...
def format_bytes(value: int) -> str:
    ...
    Quotas arrive as powers of two (10995116277760 is 10 TiB), so dividing by 1e9 renders a "10 TB"
    plan as "10995.12 GB" -- a number no customer can reconcile with anything they were sold. Units
    drifting between what we enforce and what we display is a known past bug in this ecosystem.
    """
...
def quota_exceeded_message(decision: PlanDecision) -> str:
    ...
    Says "recently deleted ... may take a few minutes" because it can be literally true: usage is
    refreshed on a timer and a denial is not re-checked live, so a customer who just freed space is
    told why they are still being refused rather than left to guess.
    ...
    return (
        ...
        f"plan with more storage. Usage is recalculated periodically, so anything you have just "
        f"deleted may take a few minutes to be reflected here."
    )
