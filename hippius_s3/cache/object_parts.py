...
class RedisObjectPartsCache:
    ...
    # EC helpers
    def build_ec_meta_key(self, object_id: str, object_version: int, part_number: int, policy_version: int) -> str:
        return f"obj:{object_id}:v:{int(object_version)}:part:{int(part_number)}:pv:{int(policy_version)}:meta"

    def build_parity_key(
        self,
        object_id: str,
        object_version: int,
        part_number: int,
        ...
        return f"obj:{object_id}:v:{int(object_version)}:part:{int(part_number)}:pv:{int(policy_version)}:stripe:{int(stripe_index)}:parity:{int(parity_index)}"

    async def set_ec_meta(
        self,
        object_id: str,
        object_version: int,
        part_number: int,
        ...
        *,
        ...
        key = self.build_ec_meta_key(object_id, object_version, part_number, policy_version)
        await self.redis.setex(key, ttl, _json.dumps(meta))

    async def get_ec_meta(
        self, object_id: str, object_version: int, part_number: int, policy_version: int
    ) -> dict | None:
        key = self.build_ec_meta_key(object_id, object_version, part_number, policy_version)
    ...
    async def set_parity_chunk(
        ...
        *,
        ...
        key = self.build_parity_key(object_id, object_version, part_number, policy_version, stripe_index, parity_index)
        await self.redis.setex(key, ttl, data)

    async def get_parity_chunk(
        self,
        object_id: str,
        object_version: int,
        part_number: int,
