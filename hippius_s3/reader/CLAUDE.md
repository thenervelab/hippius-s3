...
- [backend_fetch.py](backend_fetch.py) pulls the ciphertext from the first location that serves it into memory (one `ArionClient` per process, held in `ReplaceableHttpClient` from [`http_client.py`](../http_client.py) — same holder the peer client uses — `HIPPIUS_READ_BACKEND_FETCH_CONCURRENCY` in flight per pod, `HIPPIUS_READ_BACKEND_FETCH_ATTEMPTS` bounded retries per location; a permanent error moves to the next location). The bytes are decrypted here and yielded — never written to any cache.
