...
INSERT INTO chunk_backend (chunk_id, backend, backend_identifier, arion_hash, deleted, deleted_at)
SELECT pc.id, $3, $4, $5, false, NULL
FROM part_chunks pc
...
SET backend_identifier = COALESCE(EXCLUDED.backend_identifier, chunk_backend.backend_identifier),
    arion_hash = COALESCE(EXCLUDED.arion_hash, chunk_backend.arion_hash),
    deleted = false,
    deleted_at = NULL
RETURNING chunk_id;
