...
async def write_cache_meta(
    ...
    *,
    ...
    if ttl is not None:
        await obj_cache.set_meta(object_id, int(object_version), int(part_number), **kwargs, ttl=int(ttl))
    else:
        await obj_cache.set_meta(object_id, int(object_version), int(part_number), **kwargs)
