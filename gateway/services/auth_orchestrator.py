...
AuthMethod = Literal["access_key", "seed_phrase", "anonymous"]
...
async def authenticate_request(request: Request) -> AuthResult:
    ...
    # 3. Access Key vs Seed Phrase Branching
...
async def _authenticate_presigned_url(request: Request, logger: logging.Logger) -> AuthResult:
...
async def _authenticate_access_key_header(
    request: Request, credential: str, logger: logging.Logger
) -> AuthResult:
...
async def _authenticate_seed_phrase(request: Request, logger: logging.Logger) -> AuthResult:
