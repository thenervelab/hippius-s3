...
@pytest.mark.asyncio
async def test_proxy_startup_and_health_check(gateway_client: AsyncClient) -> None:
    """Test that gateway starts successfully and health check works."""
...
@pytest.mark.asyncio
@respx.mock
async def test_forward_simple_get_request(gateway_client: AsyncClient) -> None:
    """Test that gateway forwards a simple GET request correctly."""
    mock_backend = respx.get("http://api:8000/test-endpoint").mock(
        return_value=httpx.Response(200, json={"message": "success"})
    )

    response = await gateway_client.get("/test-endpoint")
...
@pytest.mark.asyncio
@respx.mock
async def test_forward_request_with_query_parameters(gateway_client: AsyncClient) -> None:
    """Test that gateway preserves query parameters in forwarded request."""
    ...
    response = await gateway_client.get("/list?prefix=test/&max-keys=100")
...
@pytest.mark.asyncio
@respx.mock
async def test_strip_malicious_client_headers(gateway_client: AsyncClient) -> None:
    ...
    response = await gateway_client.get("/secure", headers={"X-Hippius-Account-Id": "malicious-id-999"})
...
@pytest.mark.asyncio
@respx.mock
async def test_connection_pooling(gateway_client: AsyncClient) -> None:
    """Test that gateway reuses connections via httpx connection pooling."""
    ...
    response1 = await gateway_client.get("/endpoint1")
    response2 = await gateway_client.get("/endpoint2")
    response3 = await gateway_client.get("/endpoint3")
...
@pytest.mark.asyncio
@respx.mock
async def test_error_handling_backend_500(gateway_client: AsyncClient) -> None:
    """Test that gateway correctly forwards backend 500 errors."""
    respx.get("http://api:8000/failing-endpoint").mock(
        return_value=httpx.Response(500, json={"error": "Internal Server Error"})
    )

    response = await gateway_client.get("/failing-endpoint")
...
@pytest.mark.asyncio
@respx.mock
async def test_error_handling_backend_404(gateway_client: AsyncClient) -> None:
    """Test that gateway correctly forwards backend 404 errors."""
    respx.get("http://api:8000/nonexistent").mock(return_value=httpx.Response(404, json={"error": "Not Found"}))

    response = await gateway_client.get("/nonexistent")
...
@pytest.mark.asyncio
@respx.mock
async def test_forward_with_custom_headers(gateway_client: AsyncClient) -> None:
