...
async def test_record_cached_batch_overlap_preserves_original_age(pg_tx: asyncpg.Connection) -> None:
    await fs_cache_inventory.record_cached_batch(pg_tx, [("obj-batch-ov", 1, 0)])
    # Age the existing row: DO NOTHING must leave this cached_at untouched on re-batch.
    original = await pg_tx.fetchval(
        """
        UPDATE fs_cache_inventory SET cached_at = now() - interval '1 hour'
        WHERE object_id = 'obj-batch-ov' AND part_number = 0 RETURNING cached_at
        """
    )

    # Overlapping row (1,0) hits DO NOTHING; (1,1) is inserted fresh.
    ...
    assert count == 2
    preserved = await pg_tx.fetchval(
        "SELECT cached_at FROM fs_cache_inventory WHERE object_id = 'obj-batch-ov' AND part_number = 0"
    )
    assert preserved == original


async def test_record_cached_batch_intra_batch_duplicate_tolerated(pg_tx: asyncpg.Connection) -> None:
    # A duplicate tuple within one batch must not error (DO NOTHING absorbs it) and collapses to one row.
    await fs_cache_inventory.record_cached_batch(pg_tx, [("obj-batch-dup", 1, 0), ("obj-batch-dup", 1, 0)])

    count = await pg_tx.fetchval("SELECT count(*) FROM fs_cache_inventory WHERE object_id = 'obj-batch-dup'")
    assert count == 1
