...
def _request(node_id: str | None) -> UploadChainRequest:
...
async def test_enqueue_routes_by_the_payloads_node_and_retries_stay_on_that_node() -> None:
    # The DLQ re-queue path goes through enqueue_upload_to_backends with the stored payload, so
...
@pytest.mark.asyncio
async def test_the_loop_consumes_its_own_nodes_queue_when_node_name_is_set() -> None:
    ...
    with (
        patch.dict(os.environ, {"NODE_NAME": "ingest-node-1"}),
    ...
    assert seen == ["arion_upload_requests:ingest-node-1"]
...
async def test_a_finished_part_flips_the_drains_row_from_uploading_to_replicated() -> None:
    ...
    # SSD copy, so flipping per chunk would risk the only copy of a half-uploaded part.
    conn = AsyncMock()
    pool = MagicMock()
    ...
    uploader = Uploader(pool, FakeRedis(), FakeRedis(), cfg, backend_name="arion", backend_client=MagicMock())

    await uploader._confirm_uploaded("466916c0-d61b-4518-b81b-9576b574270a", 5, [1, 2])

    assert conn.execute.await_count == 2
    sql, object_id, version, part_number = conn.execute.await_args_list[0].args
    assert "status = 'uploading'" in sql and "SET status = 'replicated'" in sql
    assert (object_id, version, part_number) == ("466916c0-d61b-4518-b81b-9576b574270a", 5, 1)
    assert conn.execute.await_args_list[1].args[3] == 2
