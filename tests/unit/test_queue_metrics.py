...
async def test_sample_reports_depths_and_age():
    ...
    redis = FakeRedis(
        lists={
            ...
            "ovh_unpin_requests": [payload_new, payload_old],
            "arion_upload_requests:dlq": [payload_old],
        },
        zsets={"ovh_unpin_retries": ["a", "b", "c"]},
    )
    sampler = QueueDepthSampler(redis, _config(), register_metrics=False)

    await sampler.sample_once(now=1000.0)

    assert sampler.depths["ovh_unpin_requests"] == 2
    assert sampler.depths["arion_upload_requests:dlq"] == 1
    assert sampler.depths["ovh_unpin_retries"] == 3
    assert sampler.depths["arion_unpin_requests"] == 0
    assert sampler.oldest_age["ovh_unpin_requests"] == pytest.approx(100.0)
...
async def test_payload_without_timestamp_reports_no_age():
    redis = FakeRedis(lists={"ovh_unpin_requests": [json.dumps({"object_id": "x"})]})
    sampler = QueueDepthSampler(redis, _config(), register_metrics=False)

    await sampler.sample_once(now=1000.0)

    assert sampler.depths["ovh_unpin_requests"] == 1
    assert "ovh_unpin_requests" not in sampler.oldest_age


@pytest.mark.asyncio
async def test_malformed_payload_reports_no_age():
    redis = FakeRedis(lists={"ovh_unpin_requests": [b"not-json"]})
    sampler = QueueDepthSampler(redis, _config(), register_metrics=False)
...
async def test_redis_failure_keeps_previous_values():
    redis = FakeRedis(lists={"ovh_unpin_requests": [json.dumps({"first_enqueued_at": 1.0})]})
    sampler = QueueDepthSampler(redis, _config(), register_metrics=False)
