...
class StubRedis:
    """redis-accounts stand-in. One row carries plan, quota and usage -- the gate reads nothing else."""
...
def test_under_quota_allows() -> None:
    assert decide(used=1 * GB, incoming=1 * GB, limit=10 * GB).outcome == "allow"
...
def test_shadow_and_enforced_share_the_arithmetic() -> None:
    ...
    except the name of the over-quota verdict. A second copy of the arithmetic could drift, and the
    shadow period is precisely the window in which that drift would go unnoticed."""
...
def test_an_unknown_quota_allows_and_reports_catalog_miss() -> None:
    assert decide(used=999 * GB, incoming=1 * GB, limit=None).outcome == "catalog_miss"
...
def test_the_gate_touches_no_database() -> None:
    """The whole point of caching usage in the plan row: evaluate_quota is a pure function, so a
    slow or unavailable Postgres cannot affect an upload by a plan account."""
    import inspect

    assert not inspect.iscoroutinefunction(plan_gate.evaluate_quota)
    assert "db" not in inspect.signature(plan_gate.evaluate_quota).parameters
