...
async def test_the_residency_lookup_excludes_this_node() -> None:
    ...
    fetcher = PeerChunkFetcher(pool, registry, "node-a", FakeHttp(404))
...
async def test_no_peer_holds_the_part_so_the_pool_is_used() -> None:
    registry = PeerRegistry(FakeRedis(), "node-a", SELF_URL, 90)
    fetcher = PeerChunkFetcher(FakePool(None), registry, "node-a", FakeHttp(200, b"x"))
...
async def test_a_peer_that_evicted_the_chunk_returns_none() -> None:
    ...
    await PeerRegistry(redis, "node-b", PEER_URL, 90).register()
    registry = PeerRegistry(redis, "node-a", SELF_URL, 90)
    fetcher = PeerChunkFetcher(FakePool(residency_row()), registry, "node-a", FakeHttp(404))
...
async def test_a_different_part_is_looked_up_separately() -> None:
    ...
    fetcher = PeerChunkFetcher(pool, registry, "node-a", FakeHttp(200, b"x"))
...
async def test_a_saturated_peer_is_skipped_rather_than_queued() -> None:
    ...
    registry = PeerRegistry(redis, "node-a", SELF_URL, 90)
    http = BlockingHttp()
    fetcher = PeerChunkFetcher(FakePool(residency_row()), registry, "node-a", http, max_inflight=1)
...
async def test_both_shed_paths_are_recorded_with_their_reason(monkeypatch) -> None:
    ...
    capped = PeerChunkFetcher(FakePool(residency_row()), registry, "node-a", blocking, max_inflight=1)
    ...
    busy = PeerChunkFetcher(FakePool(residency_row()), registry, "node-a", FakeHttp(503), max_inflight=8)
...
async def test_the_cap_still_sheds_when_readers_genuinely_contend() -> None:
    ...
    http = BlockingHttp()
    cap = 4
    fetcher = PeerChunkFetcher(FakePool(residency_row()), registry, "node-a", http, max_inflight=cap)
...
async def test_a_short_body_legal_for_the_part_but_wrong_for_this_chunk_is_rejected() -> None:
    ...
    fetcher = PeerChunkFetcher(pool, registry, "node-a", FakeHttp(200, b"t" * 40))
...
async def test_a_wrong_length_is_counted_under_its_own_reason(monkeypatch) -> None:
    ...
    await PeerRegistry(redis, "node-b", PEER_URL, 90).register()
    registry = PeerRegistry(redis, "node-a", SELF_URL, 90)
    fetcher = PeerChunkFetcher(FakePool(sized_row([100, 100])), registry, "node-a", FakeHttp(200, b"t" * 40))
...
async def test_the_chunk_sizes_arrive_with_the_owner_in_one_query() -> None:
    ...
    registry = PeerRegistry(redis, "node-a", SELF_URL, 90)
    pool = FakePool(residency_row())
    fetcher = PeerChunkFetcher(pool, registry, "node-a", FakeHttp(200, b"peer-bytes"))
