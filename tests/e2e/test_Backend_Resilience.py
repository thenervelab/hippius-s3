"""E2E tests for backend resilience: reads recover after cache eviction."""
...
@pytest.mark.local
def test_read_recovers_after_cache_eviction(
    docker_services: Any,
    boto3_client: Any,
    unique_bucket_name: Callable[[str], str],
    cleanup_buckets: Callable[[str], None],
) -> None:
    """Upload, wait for backend, clear all caches, verify re-download via pipeline."""
    bucket = unique_bucket_name("resil-cache-evict")
