...
- Treats body as append delta; publishes only the new chunk to the backend.
...
`If-None-Match` on an append

An append only ever modifies an object that already exists, so `If-None-Match: *` — which asks for a
write that creates — can never succeed on one: an existing key answers 412 PreconditionFailed, an
absent key answers 404 NoSuchKey. The check runs inside the same row lock as the version CAS, so a
key created concurrently cannot slip past it. Any other `If-None-Match` value is 501 NotImplemented,
