...
**Postgres + 5 Redis instances + an FS chunk cache**. Two FastAPI services
...
- **Docs:** `docs/s3-2.0-cache-research.md` — "Edge (ATS) — regional read-only tier, hot objects".
...
- **5 Redis StatefulSets** — `k8s/base/redis-statefulsets.yaml`:

  | Service | Persistence | Mem cap | Purpose |
  |---|---|---|---|
  | `redis-accounts` | AOF | 2Gi | Account credit/role cache |
  | `redis-chain` | AOF | 2Gi | Chain state cache |
  ...
  The old 32Gi `redis-download-cache` (:6385) is decommissioned (2026-04-21).
...
- **Cache tier** (`k8s/cache/`) — regional read-only deployments (us-0, eu-0) of gateway+api+downloader with `HIPPIUS_READ_ONLY_MODE=true` and local cache PVCs. Currently **disabled in CI** (`if: false` in `production-deploy.yaml`).
...
### PUT
```
client → ATS (edge) → ingress → gateway(auth/ACL) → api(chunk+encrypt+FS write+DB)
   → enqueue arion_upload_requests → arion-uploader → Arion + chain
