# S3 Object Manifest Policy

Goal:
Define how objects are represented via a manifest today, and outline the target directory layout we may adopt later.

## Today (current behavior)

- Single artifact per object: a `manifest.json` describes the object and references parts by CID. Parts are identified by CIDs only.
- Chunks/parts are uploaded to the Arion backend and pinned; the database tracks `{part_number, cid, size_bytes}`.
...
- GET reads the manifest and streams bytes using part CIDs from cache or the backend.
...
- The manifest CID is stored on the object row; GET assembles bytes using part CIDs.

## Implementation Notes

- Write (today): upload+pin part CIDs; upsert parts; publish manifest; set manifest CID on object.
- Read (today): read manifest; stream from cache or backend via part CIDs.
- Filename Sanitization
  - Use the final segment of the S3 key; replace unsafe characters; preserve extension; cap length.
  - Filename visibility depends on the publish path:
...
- Helpers:
  ...
  - Backend helpers -- accept either Links (if published with a filename) or non-zero Size for the CID.
