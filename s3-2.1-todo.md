...
- **Goal 2 — Drain throughput + move ephemeral coordination off PSQL (now gate ALL uploads).**
  - **Throughput cheap wins:** batch fsync once per part + `fdatasync`; hash once during the copy stream
    (drop/sample the CephFS readback — content-addressed naming self-verifies); make `DRAIN_CONCURRENCY` env-tunable.
  - **Move the allocator's transient coordination state to Redis (promoted from Goal 3 / Tier 0.2):** the
    `cephor_leader_lease` / `cephor_node_state` / `cephor_allocation` rows are UPSERTed every ~2 s tick (autovacuum
    6995× / 3659× / 2023×). Move heartbeats + budgets + presence to Redis with TTL (expiry *is* the death signal —
    no reaper); breaks the circular Ceph-recovery dependency and kills the hot-row churn. The bigger of the four
    items; the `tick.rs:81` short-circuit (Goal 3) is the cheap interim if this slips. Full task breakdown in
    `s3-2.1-todo-next-steps.md`.
- **Goal 3 — DB hygiene (lower urgency: prod PG is NVMe).** `tick.rs:81` short-circuit unchanged allocation
  writes (cheap interim for the churn until the Goal 2 Redis move lands); wire a terminal-row GC / partition-drop
  for `cephor_replication_status`; batch the reconciler `status()` into one query.
